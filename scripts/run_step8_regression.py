...
=======================
Step 7 の統合データ（CSV）と為替データ（CSV）を読み込んで結合し、
学術論文に準拠した4つの主要感情指標および5つのコントロール変数を投入して OLS 回帰分析を行います。

投入変数:
  【主要説明変数 (4種)】
    1. text_score            (FinBERT/ModernBERT)
    2. face_negative_score   (Py-Feat: anger + disgust + fear)
    3. audio_valence         (Wav2Vec2)
    4. audio_arousal         (Wav2Vec2)

  【コントロール変数 (5種)】
    1. USDJPY_Pre_Drift      (会見前30分ドリフト bp)
    2. MPU_Japan             (日本政策不確実性指数)
    3. Rate_Change_BP        (政策金利変更幅 bp)
    4. YCC_Change_Dummy      (YCC変更ダミー)
    5. Market_Conditions     (日経平均前回会合比リターン %)
...
def main():
    ...
    # 3. マクロ・金融コントロール変数の抽出
    logger.info("コントロール変数 (USDJPY_Pre_Drift, MPU, Rate_Change, YCC, Market_Conditions) を抽出中...")
    try:
    ...
    # 5. 学術回帰分析の変数リスト決定
    # 【主要説明変数 4種】
    emotion_vars = [v for v in ['text_score', 'face_negative_score', 'audio_valence', 'audio_arousal'] if v in df_merged.columns]
    # 【コントロール変数 5種】
    controls = ['USDJPY_Pre_Drift', 'MPU_Japan', 'Rate_Change_BP', 'YCC_Change_Dummy', 'Market_Conditions']

    X_vars = emotion_vars + controls
    df_merged = df_merged.dropna(subset=['return'] + X_vars)

    if df_merged.empty:
        logger.error(f"データ結合または欠損処理によりデータが空になりました (基準日時: {args.start_time})。")
        sys.exit(1)
